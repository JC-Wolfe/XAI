...
results <- get(load("/home/jw18713/Project1/data/model_predictions/BG3_FuzzyGR.Rda")) # Marks
marks <- names(mcols(results)[1:26]) #list of names
...
pdf("/home/jw18713/Project1/Paper_Plots/Figure4/BG3_rule_circles.pdf",
...
axis(3, at=seq(1,26, by = 1), labels=F)
text(seq_along(marks),y = length(pos_df[,1]) + 1, srt = 315, adj = 1,labels = marks, xpd = TRUE,cex=0.75)
axis(2, at=seq(1,length(pos_df$ID), by = 1), labels=pos_df$ID, las=2)
...
axis(3, at=seq(1,26, by = 1), labels=F)
text(seq_along(marks),y =length(neg_df[,1]) + 0.55, srt = 315, adj = 1,labels = marks, xpd = TRUE,cex=0.75)
axis(2, at=seq(1,length(neg_df$ID), by = 1), labels=neg_df$ID, las=2)
